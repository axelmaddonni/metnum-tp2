    ...
    if (!tipoDeRanking)
    {
        ...
        if (inputParameter >> tipo_rank) {
            ...
            if (tipo_rank == 1) {
                ...
                outputFile << "& \\textbf{Equipo} & \\textbf{Puntos} \\\\ \\hline\n";
                for (int i = 0; i < size; ++i) {
                    outputFile << i+1 << " & " << equipos[R[i].primero-1] << " & " << R[i].segundo.primero << "\\\\ \\hline\n";
                }
                ...
                for (int i = 0; i < size; ++i) {
                    int j = 0;
                    while (j < size && i!=resultado[j].primero-1) {
                        j++;
                    }
                    cout << j+1 << "\n";
                }*/
